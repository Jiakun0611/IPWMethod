...
#'   D = Var_p( sum_{j in sp} d_j * pi_j * x_j )
...
#' @param lonely.psu Character option passed to `survey.lonely.psu`
#'   when `sp_des` is an ordinary survey design. Default is `"adjust"`.
#'
#' @return A `p x p` covariance matrix `D`.
#' @keywords internal
compute_D_CLW <- function(sp_des, pi_sp, Xp, lonely.psu = "adjust") {
